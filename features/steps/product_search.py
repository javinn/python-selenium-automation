...
def open_google(context):
    context.chrome_instance.get('https://www.google.com/')


@when('Input {search_word} into search field')
def input_search(context, search_word):
    search = context.chrome_instance.find_element(*SEARCH_INPUT)
    search.clear()
...
def click_search_icon(context):
    context.chrome_instance.find_element(*SEARCH_SUBMIT).click()
    sleep(1)


@then('Product results for {search_word} are shown')
def verify_found_results_text(context, search_word):
    results_msg = context.chrome_instance.find_element(*RESULTS_FOUND_MESSAGE).text
...
def verify_first_result(context, search_word):
    first_result = context.chrome_instance.find_element(*RESULTS).text
    print('\n{}'.format(first_result))
